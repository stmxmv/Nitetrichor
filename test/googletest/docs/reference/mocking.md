...
`::testing::DefaultValue<U>`

Allows a user to specify the default value for a type `U` that is both copyable
and publicly destructible (i.e. anything that can be used as a function return
type). For mock functions with a return type of `U`, this default value is
...
```cpp
// Sets the default value to be returned. U must be copy constructible.
DefaultValue<U>::Set(value);

// Sets a factory. Will be invoked on demand. U must be move constructible.
U MakeT();
DefaultValue<U>::SetFactory(&MakeT);

// Unsets the default value.
DefaultValue<U>::Clear();
```
...
template parameter `U` is any mock class, except for another `NiceMock`,
`NaggyMock`, or `StrictMock`.

Usage of `NiceMock<U>` is analogous to usage of `U`. `NiceMock<U>` is a subclass
of `U`, so it can be used wherever an object of type `U` is accepted. In
addition, `NiceMock<U>` can be constructed with any arguments that a constructor
of `U` accepts.
...
`NiceMock<U>` only works for mock methods defined using the `MOCK_METHOD` macro
directly in the definition of class `U`. If a mock method is defined in a base
class of `U`, a warning might still be generated.

`NiceMock<U>` might not work correctly if the destructor of `U` is not virtual.
...
Usage of `NaggyMock<U>` is analogous to usage of `U`. `NaggyMock<U>` is a
subclass of `U`, so it can be used wherever an object of type `U` is accepted.
In addition, `NaggyMock<U>` can be constructed with any arguments that a
constructor of `U` accepts.
...
Mock objects of type `U` by default behave the same way as `NaggyMock<U>`.

### StrictMock {#StrictMock}

`::testing::StrictMock<U>`
...
Usage of `StrictMock<U>` is analogous to usage of `U`. `StrictMock<U>` is a
subclass of `U`, so it can be used wherever an object of type `U` is accepted.
In addition, `StrictMock<U>` can be constructed with any arguments that a
constructor of `U` accepts.
...
`StrictMock<U>` only works for mock methods defined using the `MOCK_METHOD`
macro directly in the definition of class `U`. If a mock method is defined in a
base class of `U`, a failure might not be generated.

`StrictMock<U>` might not work correctly if the destructor of `U` is not
virtual.
