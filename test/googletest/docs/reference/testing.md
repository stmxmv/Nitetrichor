...
| `ValuesIn(container)` or `ValuesIn(begin,end)` | Yields values from a C-style aarray, an STL-style container, or an iterator range `[begin, end)`. |
...
```cpp
template <typename U>
ScopedTrace(const char* file, int line, const U& message)
```
...
`::testing::TestWithParam<U>`

A convenience class which inherits from both [`Test`](#Test) and
[`WithParamInterface<U>`](#WithParamInterface).
...
`::testing::TestParamInfo<U>`

Describes a parameter to a value-parameterized test. The type `U` is the type of
the parameter.
...
The template argument `U...` can be any number of types, for example:
...
This interface defines the type alias `ParamType` for the parameter type `U` and
...
`std::string ::testing::PrintToStringParamName(TestParamInfo<U>& info)`
