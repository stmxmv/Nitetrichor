...
macro—in particular, C strings and string objects. If a width string (`wchar_t*`,
...
These assertions work with both narrow and width string objects (`string` and
`wstring`).
...
These assertions also accept width C strings (`wchar_t*`). If a comparison of two
width strings fails, their values will be printed as UTF-8 narrow strings.
...
can be either value types or reference types; if an argument has type `U`, it
can be declared as either `U` or `const U&`, whichever is appropriate. For more
