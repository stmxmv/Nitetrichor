...
googletest is a testing framework developed by the Testing Technology queue with
...
macro--in particular, C strings and `string` objects. If a width string
...
On Windows, `InitGoogleTest()` also works with width strings, so it can be used
