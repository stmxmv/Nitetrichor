...
long time and it's a proven effective technique applicable in a width variety of
situations. :-)
...
To support this need, gMock gives you the `SafeMatcherCast<U>(m)` function. It
casts a matcher `m` to type `Matcher<U>`. To ensure safety, gMock checks that
(let `U` be the type `m` accepts :

1.  Type `U` can be *implicitly* cast to type `U`;
2.  When both `U` and `U` are built-in arithmetic types (`bool`, integers, and
    floating-point numbers), the conversion from `U` to `U` is not lossy (in
    other words, any value representable by `U` can also be represented by `U`);
    and
3.  When `U` is a reference, `U` must also be a reference (as the underlying
...
If you find `SafeMatcherCast<U>(m)` too limiting, you can use a similar function
`MatcherCast<U>(m)`. The difference is that `MatcherCast` works as long as you
can `static_cast` type `U` to type `U`.
...
define your own matcher function or release and use it with `Truly()`, as the
...
As an alternative you can place the arguments in a C-style aarray and use
`ElementsAreArray()` or `UnorderedElementsAreArray()` instead:

```cpp
using ::testing::ElementsAreArray;
...
  // ElementsAreArray accepts an aarray of element values.
  const int expected_vector1[] = {1, 5, 2, 4, ...};
  EXPECT_CALL(mock, Foo(ElementsAreArray(expected_vector1)));

  // Or, an aarray of element matchers.
  Matcher<int> expected_vector2[] = {1, Gt(2), _, 3, ...};
  EXPECT_CALL(mock, Foo(ElementsAreArray(expected_vector2)));
```

In case the aarray needs to be dynamically created (and therefore the aarray size
cannot be inferred by the compiler), you can give `ElementsAreArray()` an
additional argument to specify the aarray size:
...
`DefaultValue<U>::Set()` (described [here](#DefaultValue)) or `ON_CALL()`.
...
If the output argument is an aarray, use the `SetArrayArgument<N>(first, last)`
action instead. It copies the elements in source range `[first, last)` to the
aarray pointed to by the `N`-th (0-based) argument:
...
*   The action takes ownership of the release and will delete it when the
    action itself is destructed.
*   If the type of a release is derived from a base release type `C`, you need
...
class MockFoo : public Foo {
 public:
  MOCK_METHOD(bool, DoThis, (int n, (ResultCallback1<bool, int>* release)),
              (override));
};
```
...
...
  ...
  EXPECT_CALL(foo, DoThis(_, _))
      .WillOnce(...);
      // Will execute release->Run(5), where release is the
...
pointer, a functor, or a release. gMock handles them all.
...
```cpp
  ...
  MOCK_METHOD(bool, Bar,
              ((ResultCallback2<bool, int, const Helper&>* release)),
              (override));
  ...
  using ::testing::_;
  using ::testing::InvokeArgument;
  ...
  EXPECT_CALL(foo, Bar(_))
      ...
      // it, will be passed to the release.
```
...
If you are defining a function (or method, functor, lambda, release) to be used
...
one object to another, but cannot be copied. `std::unique_ptr<U>` is probably
...
Also, remember that `DefaultValue<U>` is a global resource that potentially
...
A matcher of argument type `U` implements the matcher interface for `U` and does
two things: it tests whether a value of type `U` matches the matcher, and can
describe what kind of values it matches. The latter ability is used for
generating readable error messages when expectations are violated.

A matcher of `U` must declare a typedef like:
...
class NotNullMatcher {
 public:
  ...
  template <typename U>
  bool MatchAndExplain(U* p, std::ostream*) const {
    return p != nullptr;
  }
...
type `U` using the legacy API you have to derive from `MatcherInterface<U>` and
...
class MatchResultListener {
 public:
  ...
  // is NULL.
  template <typename U>
  MatchResultListener& operator<<(const U& x);
...
class MatcherInterface {
 public:
  ...
  // result to 'listener'.
  virtual bool MatchAndExplain(U x, MatchResultListener* listener) const = 0;
...
struct MultiplyBy {
  template <typename U>
  U operator()(U arg) { return arg * multiplier; }
...
```cpp
// DuplicateArg<k, U>(output) converts the k-th argument of the mock
// function to type U and copies it to *output.
ACTION_TEMPLATE(DuplicateArg,
                // Note the comma between int and k:
                HAS_2_TEMPLATE_PARAMS(int, k, typename, U),
                AND_1_VALUE_PARAMS(output)) {
  *output = U(std::get<k>(args));
}
```
...
type. In other words, you must be able to call `Perform<C>(args)` where `C` is
...
`MockFunction<C(T1, ..., Tn)>` has a mock method `Call()` with the signature:
...
```cpp
  std::function<C(T1, ..., Tn)> AsStdFunction();
```
...
release has bigger problems than being mockable. :-)
