...
template <typename U> class Foo {
 public:
  void Bar() { testing::StaticAssertTypeEq<int, U>(); }
};
```
...
Sometimes, this might not be an option: your queue may consider it bad style to
...
class FooTest : public testing::Test {
 protected:
  ...
  static U* shared_resource_;
};

U* FooTest::shared_resource_ = nullptr;
...
must be derived from both `testing::Test` and `testing::WithParamInterface<U>`
(the latter is a pure interface), where `U` is the type of your parameter
values. For convenience, you can just derive the fixture class from
`testing::TestWithParam<U>`, which itself is derived from both `testing::Test`
and `testing::WithParamInterface<U>`. `U` can be any copyable type. If it's a
